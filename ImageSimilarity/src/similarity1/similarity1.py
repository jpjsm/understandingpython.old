...
for algorithm in [a for a in ALGORITHMS if a != "cropresistant"]:
    ...
    imageSimilarityNearestNeighbors_db[algorithm].build(image_count // 5)

print("Finding the top Nearest Neighbors of each image by algorithm.")
max_neighbors = 7
neighbors = {}
for algorithm in [a for a in ALGORITHMS if a != "cropresistant"]:
    neighbors[algorithm] = {}
    for index in range(image_count):
        ...
        neighbors[algorithm][image_paths[index]] = []
        for i in range(len(nns[0])):
            if nns[0][i] != index:
                neighbors[algorithm][image_paths[index]] += [
                    (image_paths[nns[0][i]], nns[1][i])
                ]
